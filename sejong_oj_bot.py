...
def help_command(bot, update):
    sejongBot = update.message.chat_id  # chat_id : 연결고리
    bot.send_message(chat_id = sejongBot, text =emojize("도움이 필요한 문제의 번호를 알려주시면\n해당 문제의 힌트를 제공해드립니다:wink:."
                                                        "\n한 번에 한 문제씩 입력해주시면 고마울 것 같아요!"
                                                        "\n저는 지금 30개의 문제를 알고 있어요!"
                                                        "\n예시) 소융대봇아 나 3번 문제 좀 도와줘 ㅠㅠ",use_aliases=True))
    bot.send_message(chat_id=sejongBot, text=emojize("/relevant : 해당 문제와 관련된 문제들을 보여줍니다."
                                                     "\n/bestAnswer : 실습 기간이 지난 문제의 모범 답안을 보여줍니다."
                                                     "\n/call : 조교님을 즉시 호출합니다."
                                                     "\n/end : 수정된 파일을 저장합니다."))
...
def random_command(bot, update):
    sejongBot = update.message.chat_id  # chat_id : 연결고리
    bot.send_message(chat_id=sejongBot, text="제가 알고 있는 문제 중에 랜덤한 문제를 드릴게요 !")
    num = random.randrange(1, 30)
    bot.send_message(chat_id=sejongBot, text=str(num)+'번을 풀어보세요 !')
    bot.send_photo(chat_id=sejongBot, photo=open('image\\'+str(num)+'.jpg', 'rb'))
    return
